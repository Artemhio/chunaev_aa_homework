...
## Работа с операциями: поиск, категории и интерфейс

В проект добавлены функции для обработки банковских операций:

### 1. Поиск по описанию (`process_bank_search`)
Функция принимает список транзакций и строку поиска.  
Возвращает только те операции, в чьём `description` встречается слово поиска.
Поиск нечувствителен к регистру и основан на `re`.

### 2. Подсчёт количества операций по категориям (`process_bank_operations`)
Функция принимает список операций и список категорий.  
Возвращает словарь вида `{категория: количество}`.  
Для подсчёта используется `Counter`.

### 3. Основной пользовательский интерфейс (`main`)
Функция `main` связывает функциональности проекта:
- загрузка транзакций из JSON/CSV/XLSX
- фильтрация по статусу
- сортировка по дате
- фильтр по рублевым операциям
- поиск по слову в описании
- печать итогового списка

### 4. Новые модули
- `operations_processing.py` — поиск и подсчёт категорий
